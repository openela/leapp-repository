...
class NetworkManagerUpdateConfig(Actor):
    """
    Updates NetworkManager configuration for OpenELA Linux 8.

    On OpenELA Linux 7 NetworkManager uses the "dhclient" DHCP backend by default, while
    the default is "internal" on OpenELA Linux 8. We want to keep "dhclient" enabled on
